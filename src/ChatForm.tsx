...
export default function ChatForm() {
  ...
  function defineFileType(file: ExtFileType) {
    ...
    if (file.type.includes("audio")) {
      return (
        <audio className="form_chat__file" src={filePath} controls></audio>
      );
    }

    if (file.type.includes("video")) {
      return (
        <video className="form_chat__file" src={filePath} controls></video>
      );
    }
  ...
  return (
    <>
      ...
      >
        ...
        <input
          accept=".png, .mp4, .mp3, .ogg, .wav, .m4v"
      ...
      {files.length > 0 && (
        ...
        >
          <div className="popup_file__wrapper">
            {!initiateUplaod ? (
              <>
                <TableComp items={files} renderItem={(file) => {
                  return (
                    <li key={file._id}>
                      <div>
                        <span>{file.name}</span>
